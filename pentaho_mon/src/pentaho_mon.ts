...
//exec SQL sur la DB pentaho EAI
async function selectFromPentaho(sql: string) {
	let connection;
	try {
...
//exec SQL sur la DB Blueway PROD 
async function selectFromBlueway(sql: string) {
	let connection;
	try {
		connection = await oracledb.getConnection(
			{
...
async function sleep_min(min: number) {
...
//VTOM API
async function getVtom(endpoint: string) {
...
async function checkVtomRessources(){
	...
	const out_data:types.application[] = []
	for(const ressource of ressources as [types.ressource] )
	{
		if(ressource.type=='Weight' && ressource.value =='0'){
			const data:types.application={
...
// verrification de la disponibilite des endpoints
async function load_endpoints(path: string) {
	let endpoints:Array<types.endpoint> = []
...
//Blueway API
async function getBlueway(searchparam: URLSearchParams) {
	const httpsAgent = new https.Agent({
		keepAlive: true,
		rejectUnauthorized: false
	});
...
async function checkBluewaySvc(){
	const services = await getBlueway(new URLSearchParams({
		...
		flowType: 'EAII',
		actionXML: 'launch',
		var_isService: '1' // passer a 0 pour recuperer les wkf
	}))
	const parser = new XMLParser()
	const serv_obj:types.BwySvc= parser.parse(services) 

	return serv_obj.SCR_Svc_ServiceManager_getNodes_OUTPUT.SCR_WRK_TABLE_services.SCR_WRK_TABLE_services
}
// ecriture d'un array dans un fichier de log
async function array_to_file(array:Array<any>, path: string){
